...
pub(crate) struct NodeConfig {
    ...
    group_peer_addrs: BTreeMap<PublicKey, SocketAddr>,
...
impl Default for NodeConfig {
    fn default() -> Self {
        ...
        let mut group_peer_addrs = BTreeMap::new();
...
impl NodeConfig {
        ...
        let config = match &cli.config {
            Some(path) => {
                let settings = config::Config::builder()
                    ...
                    .add_source(config::Environment::with_prefix("GBFT"))
                    .build()
                    .unwrap();
    ...
    pub fn get_group_peer_addrs(&self) -> HashMap<PublicKey, SocketAddr> {
      let mut ret = HashMap::new();
      for (k, v) in &self.group_peer_addrs {
          ret.insert(*k, *v);
      }
      ret
  }
    ...
    pub fn get_group_voter_set(&self) -> VoterSet {
      let v: Vec<_> = self.group_peer_addrs.keys().cloned().collect();
      VoterSet::new(v)
  }
    ...
    pub fn override_group_voter_set(&mut self, voter_set: &VoterSet) {
      let mut group_peer_addrs = BTreeMap::new();
      voter_set.iter().for_each(|id| {
          group_peer_addrs.insert(
              *id,
      ...
      self.group_peer_addrs = group_peer_addrs;
  }
    ...
    pub fn set_group_peer_addrs(&mut self, group_peer_addrs: BTreeMap<PublicKey, SocketAddr>) {
      self.group_peer_addrs = group_peer_addrs;
  }
